...
## 2. 고해상도 실제 지도 & 위성사진 엔진 (`MapScreen.tsx`)
- **실제 고해상도 타일 맵 도입 (Leaflet Engine)**:
  - 기존의 단순 일러스트 대신 **실제 서울대공원·청계호수·국립현대미술관의 정밀 지도**를 고해상도로 렌더링.
  - **[🗺️ 일반 정밀 지도 (CartoDB Voyager)] ⇄ [🛰️ 실제 고해상도 항공 위성사진 (Esri World Imagery)]** 원클릭 전환 기능 탑재.
- **실시간 GPS 및 코스 트랙선**:
  - 호수둘레길(하늘색) / 산림욕장길(초록색) 실제 GPS 경로선(Polyline) 및 공통 출발지(코끼리열차 매표소) 마커.
...
## 5. 완료된 기능 현황 ✅
- [x] **고해상도 실제 지도 및 실제 항공 위성사진 전환 엔진 구축 (`MapScreen.tsx`)**
