...
| **💻 2. 운영본부 관리자용** | **[https://mission-connect-final.vercel.app/admin](https://mission-connect-final.vercel.app/admin)** | • 운영진 / MC / 저녁 행사용 종합 관제 센터<br>• 🎯 저녁 퀴즈 마스터 모드 (플래시카드/블라인드)<br>• 🧭 **Discovery 퀴즈 관리 & 실시간 테스트 풀이/통계**<br>• 💡 50명 참가자 7문항 답변 실시간 조회 & CSV<br>• 💬 조별 People Quest 추천 현황 & CSV<br>• 📊 실시간 조별 순위 현황<br>• 📥 저녁 퀴즈용 마스터 결합 엑셀(CSV) 다운로드 |

---

## 2. Activity 2: Discovery Quiz & 순환 코스 개편 체계
1. **코끼리열차 매표소 기점 순환 회귀 동선 (`MapScreen.tsx`)**:
   - **🌊 호수둘레길 순환 (4~6조, 2.8km)**: 코끼리열차 매표소(출발) ➡️ 호수 북측 ➡️ **국립현대미술관(📸 단체사진)** ➡️ 호수 브릿지 ➡️ 테마가든 장미원 ➡️ 코끼리열차 매표소(도착/회귀).
   - **🦁 동물원둘레길 순환 (1~3조, 4.5km)**: 코끼리열차 매표소(출발) ➡️ 미술관 셔틀로 ➡️ **국립현대미술관(📸 단체사진)** ➡️ 동물원둘레길 피톤치드 쉼터 ➡️ 동물원 외곽 순환로 ➡️ 코끼리열차 매표소(도착/회귀).
   - 공통 기점 핀: **"🚊 코끼리열차 매표소 [출발/도착]"**.
   - 공통 단체사진 촬영지 배지: **"📸 단체사진 촬영지 (미술관 앞)"**.
2. **조당 딱 3문항 (공통 2개 + 코스 전용 1개, 총 300pt) (`DiscoveryQuizScreen.tsx`)**:
   - **[공통 1]**: 코끼리열차 매표소 앞 광장 (코끼리열차 최초 개통 1984년)
   - **[공통 2]**: 국립현대미술관 과천관 앞 (📸 단체사진 촬영지 & 노래하는 거인상 Singing Man)
   - **[동물원둘레길 전용 1] (1~3조)**: 동물원둘레길 피톤치드 숲길 쉼터 (피톤치드)
   - **[호수둘레길 전용 1] (4~6조)**: 대공원 호수 브릿지 전망 데크 (청계저수지)
3. **500pt 만점 밸런스**:
   - People Quest (조별 대화 1인 추천): 200pt
   - Discovery Quiz (현장 객관식 3문항): 300pt
   - **총 개인 & 조별 만점: 500pt**
4. **관리자 Discovery 퀴즈 센터 (`AdminScreen.tsx`)**:
   - 4개 문항 상세 내용, GPS 좌표, 인증 반경, 정답 및 해설 실시간 조회.
   - 관리자 즉시 테스트 풀이 & 채점 기능 (GPS 우회 없이 바로 검증 가능).
   - 문항별 실시간 풀이 인원 수, 정답률, 조별 참여 분포 실시간 집계.

---

## 3. 완료된 기능 현황 ✅
- [x] **동물원둘레길 순환 트랙선 & 국립현대미술관 단체사진 촬영지 핀 연동 (`MapScreen.tsx`)**
- [x] **관리자 페이지 내 Discovery 퀴즈 관리 & 실시간 테스트 풀이/통계 센터 구축 (`AdminScreen.tsx`)**
